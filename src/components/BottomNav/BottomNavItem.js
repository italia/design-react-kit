...
class BottomNavItem extends React.Component {
  render () {
    ...
    const activeClass = classNames(
      this.props.active ? 'active' : false
    )
    const badgeWrapper = (this.props.badge)
      ? <div className='badge-wrapper'>
        <span className='bottom-nav-badge'>{this.props.badge}</span>
      </div> : false
    const alertWrapper = (this.props.alert) ? <div className='badge-wrapper'>
    ...
    return (
      <li {...attributes}>
        <Tag href={this.props.link} className={activeClass}>
          ...
          <span className='bottom-nav-label'>{this.props.label}<span className='sr-only'>{this.props.srText}</span></span>
        </Tag>
      </li>
