...
type TypeProps = Pick<InputProps, 'plaintext' | 'type'> & {staticInput?: boolean}
type FormControlProps = Pick<InputProps, 'addon'> & TypeProps;

function getFormControlClassInternal({ plaintext, staticInput, type= 'text', addon }: FormControlProps) {
...
export function getInfoTextControlClass({ valid, invalid }: ValidationProps, cssModule?: CSSModule) {
...
type InputClassesParams = ValidationProps & Pick<InputProps, 'bsSize' | 'placeholder' | 'value' | 'label' | 'infoText' | 'normalized' > & {
  isFocused: boolean,
