...
<div className='docs'>
  <Callout color='info'>
    <CalloutText>
      L'hook <code>useNavScroll</code> utilizza l'API native{' '}
      <code>IntersectionObserver</code> supportata da browser moderni. Qualora
      il browser richiesto non lo supportasse (ad esempio Internet Explorer 11)
...
<div className='docs'>
  <Callout color='info'>
    <CalloutText>
      Ogni link all’interno della Navscroll contiene l’attributo{' '}
      <code>href</code> con il valore dell’id relativo all’elemento correlato in
      pagina: <code>href="#idElemento"</code>.
    </CalloutText>
  </Callout>
</div>
