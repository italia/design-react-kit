import { an as __toESM, a2 as $B, a0 as KB, N as NI, ao as MarkupIcon, ap as debounce2, aq as getControlId, ar as __commonJS } from "./index-BVfv5SMh.js";
import { e, r as reactExports } from "./index-BehpJNG5.js";
import { T as Tr } from "./index-D7280_ap.js";
...
var Wrapper = Tr.div({ position: "relative", maxWidth: 250, '&[aria-readonly="true"]': { opacity: 0.5 } }), PickerTooltip = Tr($B)({ position: "absolute", zIndex: 1, top: 4, left: 4, "[aria-readonly=true] &": { cursor: "not-allowed" } }), TooltipContent = Tr.div({ width: 200, margin: 5, ".react-colorful__saturation": { borderRadius: "4px 4px 0 0" }, ".react-colorful__hue": { boxShadow: "inset 0 0 0 1px rgb(0 0 0 / 5%)" }, ".react-colorful__last-control": { borderRadius: "0 0 4px 4px" } }), Note = Tr(KB)(({ theme }) => ({ fontFamily: theme.typography.fonts.base })), Swatches = Tr.div({ display: "grid", gridTemplateColumns: "repeat(9, 16px)", gap: 6, padding: 3, marginTop: 5, width: 200 }), SwatchColor = Tr.div(({ theme, active }) => ({ width: 16, height: 16, boxShadow: active ? `${theme.appBorderColor} 0 0 0 1px inset, ${theme.textMutedColor}50 0 0 0 4px` : `${theme.appBorderColor} 0 0 0 1px inset`, borderRadius: theme.appBorderRadius })), swatchBackground = `url('data:image/svg+xml;charset=utf-8,<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill-opacity=".05"><path d="M8 0h8v8H8zM0 8h8v8H0z"/></svg>')`, Swatch = ({ value, style, ...props }) => {
...
}, Input = Tr(NI.Input)(({ theme, readOnly }) => ({ width: "100%", paddingLeft: 30, paddingRight: 30, boxSizing: "border-box", fontFamily: theme.typography.fonts.base })), ToggleIcon = Tr(MarkupIcon)(({ theme }) => ({ position: "absolute", zIndex: 1, top: 6, right: 7, width: 20, height: 20, padding: 4, boxSizing: "border-box", cursor: "pointer", color: theme.input.color })), ColorSpace = ((ColorSpace2) => (ColorSpace2.RGB = "rgb", ColorSpace2.HSL = "hsl", ColorSpace2.HEX = "hex", ColorSpace2))(ColorSpace || {}), COLOR_SPACES = Object.values(ColorSpace), COLOR_REGEXP = /\(([0-9]+),\s*([0-9]+)%?,\s*([0-9]+)%?,?\s*([0-9.]+)?\)/, RGB_REGEXP = /^\s*rgba?\(([0-9]+),\s*([0-9]+),\s*([0-9]+),?\s*([0-9.]+)?\)\s*$/i, HSL_REGEXP = /^\s*hsla?\(([0-9]+),\s*([0-9]+)%,\s*([0-9]+)%,?\s*([0-9.]+)?\)\s*$/i, HEX_REGEXP = /^\s*#?([0-9a-f]{3}|[0-9a-f]{6})\s*$/i, SHORTHEX_REGEXP = /^\s*#?([0-9a-f]{3})\s*$/i, ColorPicker = { hex: Z, rgb: He, hsl: ue }, fallbackColor = { hex: "transparent", rgb: "rgba(0, 0, 0, 0)", hsl: "hsla(0, 0%, 0%, 0)" }, stringToArgs = (value) => {
  ...
  return e.createElement(Wrapper, { "aria-readonly": readonly }, e.createElement(PickerTooltip, { startOpen, trigger: readonly ? [null] : void 0, closeOnOutsideClick: true, onVisibleChange: () => addPreset(color), tooltip: e.createElement(TooltipContent, null, e.createElement(Picker, { color: realValue === "transparent" ? "#000000" : realValue, onChange: updateValue, onFocus, onBlur }), presets.length > 0 && e.createElement(Swatches, null, presets.map((preset, index) => e.createElement($B, { key: `${preset.value}-${index}`, hasChrome: false, tooltip: e.createElement(Note, { note: preset.keyword || preset.value }) }, e.createElement(Swatch, { value: preset[colorSpace], active: color && id(preset[colorSpace]) === id(color[colorSpace]), onClick: () => updateValue(preset.value) }))))) }, e.createElement(Swatch, { value: realValue, style: { margin: 4 } })), e.createElement(Input, { id: getControlId(name), value, onChange: (e2) => updateValue(e2.target.value), onFocus: (e2) => e2.target.select(), readOnly: readonly, placeholder: "Choose color..." }), value ? e.createElement(ToggleIcon, { onClick: cycleColorSpace }) : null);
