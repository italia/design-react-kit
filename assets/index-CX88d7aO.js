...
var SIGNATURE_REGEXP = /^\(.*\) => /, convert3 = (type) => {
  ...
  switch (typeof raw < "u" && (base.raw = raw), name2) {
    ...
    case "elementType":
    default: {
      if ((name2 == null ? void 0 : name2.indexOf("|")) > 0) try {
...
var convert4 = (docgenInfo) => {
  ...
  try {
    if (type != null) return convert3(type);
    if (tsType != null) return convert(tsType);
    if (flowType != null) return convert2(flowType);
  } catch (err) {
    console.error(err);
  }
  return null;
...
function createDefaultValue(defaultValue, type) {
  if (defaultValue != null) {
    ...
    if (!isDefaultValueBlacklisted(value)) return isTooLongForDefaultValueSummary(value) ? createSummaryValue(type == null ? void 0 : type.name, value) : createSummaryValue(value);
  }
  return null;
}
...
export {
  ...
  isFunction$3 as k,
  hasDocgen as l,
  extractComponentProps as m,
  getDocgenSection as n,
